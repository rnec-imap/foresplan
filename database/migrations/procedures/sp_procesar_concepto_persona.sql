CREATE OR REPLACE FUNCTION public.sp_procesar_concepto_persona(p_id_periodo character varying)
...
begin
	...
	select id_persona,id_tipo_planilla,id_ubicacion
	from persona_detalles 
	...
	Loop
		...
		values (p_id_periodo::int,v_id_ubicacion,v_id_planilla,v_id_subplanilla,cursor_personas.id_persona,v_ano_peri_tpe,'1',now(),now());
		...
		where t2.id=cursor_personas.id_tipo_planilla
		
		loop
			v_mont_fijo_cmf := null;
			if cursor_conceptos.id_concepto = 27 then 
				select mont_cont_ctr into v_mont_fijo_cmf from contratos where id_persona = cursor_personas.id_persona order by id desc limit 1;
			end if;
			if cursor_conceptos.id_concepto = 39 then
				v_mont_fijo_cmf := 1025;
			end if;
			...
			values (p_id_periodo::int,cursor_conceptos.id_planilla,cursor_conceptos.id_subplanilla,cursor_personas.id_persona,cursor_personas.id_ubicacion,cursor_conceptos.id_concepto,v_mont_fijo_cmf,'1',now(),now());
