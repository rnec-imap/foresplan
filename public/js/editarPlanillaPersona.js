...
$(document).ready(function () {
	...
	$('#btnNuevoConcepto').click(function () {
		...
		//alert(id_persona);
		//modalConceptoPersona(0,id_ubicacion,id_planilla,id_subplanilla,id_persona);
		modalConceptoPersona(0,id_periodo,id_persona)
	});
	
	$('#btnNuevoConceptoResumen').click(function () {
		var id_persona = $('#id_persona').val();
		modalConceptoPersonaResumen(0,id_planilla,id_subplanilla,id_persona);
	});
...
function cargarConceptoPersona(id_periodo,id_persona){

	$("#divConceptoPersona").hide();
	...
	$.ajax({
			url: "/planilla/obtener_concepto_persona/"+id_periodo+"/"+id_persona,
			type: "GET",
			success: function (result) {  
					//$('#id_solicitud').val(idSolicitud);
					$("#divConceptoPersona").show();
...
function cargarConceptoPersonaResumen(id_planilla,id_subplanilla,id_persona){

	$("#divConceptoPersonaResumen").hide();
    $("#tblConceptosPersonaResumen tbody").html("");
	
	$('#id_persona').val(id_persona);
	
	$.ajax({
			url: "/planilla/obtener_concepto_persona_resumen/"+id_planilla+"/"+id_subplanilla+"/"+id_persona,
			type: "GET",
			success: function (result) {  
					//$('#id_solicitud').val(idSolicitud);
					$("#divConceptoPersonaResumen").show();
					$("#tblConceptosPersonaResumen tbody").html(result);
			}
	});
...
function eliminarConceptoPersona(id){
    ...
    $.ajax({
            ...
            success: function (result) {
				...
				var id_subplanilla = $('#id_subplanilla').val();
				var id_persona = $('#id_persona').val();
				cargarConceptoPersona(id_periodo,id_persona);
            }
    });
}

function eliminarMetaPersona(id){
	
	var id_periodo = $('#id_periodo').val();
	
    $.ajax({
            url: "/planilla/eliminar_meta_persona/"+id,
            type: "GET",
            success: function (result) {
				/*
				var id_planilla = $('#id_planilla').val();
				...
				*/
				//alert("ok");
				cargarMetasPersona(id_periodo);
            }
    });
}

function modalConceptoPersona(id,id_periodo,id_persona){
	    // modalConceptoPersona(0,id_ubicacion,id_planilla,id_subplanilla,id_persona);
	//(0,id_ubicacion,id_planilla,id_subplanilla,id_persona);
	
	$(".modal-dialog").css("width","85%");
	$('#openOverlayOpc .modal-body').css('height', 'auto');

	$.ajax({
			url: "/planilla/modal_concepto_persona/"+id+"/"+id_periodo+"/"+id_persona,
			type: "GET",
			success: function (result) {  
					$("#diveditpregOpc").html(result);
					$('#openOverlayOpc').modal('show');
...
function modalConceptoPersonaResumen(id,id_planilla,id_subplanilla,id_persona){
	
	$(".modal-dialog").css("width","85%");
	$('#openOverlayOpc .modal-body').css('height', 'auto');

	$.ajax({
			url: "/planilla/modal_concepto_persona_resumen/"+id+"/"+id_planilla+"/"+id_subplanilla+"/"+id_persona,
			type: "GET",
			success: function (result) {  
					$("#diveditpregOpc").html(result);
					$('#openOverlayOpc').modal('show');
...
function modalPersona(){
	...
	$.ajax({
			url: "/planilla/modal_persona/"+id_periodo,
			type: "GET",
			success: function (result) {  
					$("#diveditpregOpc").html(result);
					$('#openOverlayOpc').modal('show');
...
function cargarMetasPersona(){
	//alert(id_ubicacion+','+id_planilla+','+id_subplanilla+','+anio+','+id_mes);
	//return false;
	var id_periodo = $('#id_periodo').val();
	
	//$('#tblMetasPersona').dataTable().fnDestroy();
    $("#tblMetasPersona tbody").html("");
	$.ajax({
			url: "/planilla/obtener_metas_persona/"+id_periodo,
			type: "GET",
			success: function (result) { 
					//alert("ok");
					//$('#id_solicitud').val(idSolicitud);
					
					$('#tblMetasPersona').dataTable().fnDestroy(); //la destruimos
					$('#tblMetasPersona tbody').html("");
					
					$("#tblMetasPersona tbody").html(result);
					/*
					$('#tblMetasPersona').DataTable({
						"searching": false,
						"info": false,
						"bSort" : false,
					...
					*/
					...
					$("#system-search").keyup(function() {
						var dataTable = $('#tblMetasPersona').dataTable();
