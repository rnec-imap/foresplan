...
function guardarPlanilla(){
	...
	var id_subplanilla = $('#id_subplanilla').val();
	var anio = $('#anio').val();
	var mes = $('#mes').val();
	var fech_inic_tpe = $('#fech_inic_tpe').val();
	var fech_fina_tpe = $('#fech_fina_tpe').val();
	var rd_tipo = $('input[name=rd_tipo]:checked').val();
	
	if(id_ubicacion == "0")msg+="Debe seleccionar una Empresa <br>";
	if(id_planilla == "0")msg+="Debe seleccionar una Planilla <br>";
	if(id_subplanilla == "0")msg+="Debe seleccionar una SubPlanilla <br>";
	if(anio == "0")msg+="Debe seleccionar un Año <br>";
	if(mes == "0")msg+="Debe seleccionar un Mes <br>";
	if(fech_inic_tpe == "")msg += "Debe ingresar la fecha de inicio <br>";
	if(fech_fina_tpe == "")msg += "Debe ingresar la fecha de fin <br>";
	if(!$('input[name=rd_tipo]').is(':checked'))msg += "Debe selececcionar si es general o adicional <br>";
    ...
    $.ajax({
            ...
            success: function (result) {
				//return false;
				location.href="agregar_persona_planilla/"+result;
            }
    });
}
...
function cargarDesembolso(idSolicitud){

    $("#divDesembolso").html("");
	$.ajax({
			url: "/prestamo/obtener_desembolso/"+idSolicitud,
			type: "GET",
			success: function (result) {  
					$('#id_solicitud').val(idSolicitud);
					$("#divDesembolso").html(result);
			}
	});

}

function cargarConceptoPersona(id_planilla,id_subplanilla,id_persona){

    $("#tblCronograma tbody").html("");
	$.ajax({
			url: "/planilla/obtener_concepto_persona/"+id_planilla+"/"+id_subplanilla+"/"+id_persona,
			type: "GET",
			success: function (result) {  
					//$('#id_solicitud').val(idSolicitud);
					$("#tblCronograma tbody").html(result);
			}
	});
...
function fn_save_desembolso(id_estado){
    
	var tabla = $('#tabla').val();
	var columna = $('#columna').val();
	//alert(idSolicitud);return false;
    $.ajax({
			url: "/prestamo/send",
            type: "POST",
            //data : $("#frmCita").serialize()+"&id_medico="+id_medico+"&fecha_cita="+fecha_cita,
            data : $("#frmSolicitud").serialize()+"&id_estado="+id_estado,
            success: function (result) {
					datatablenew();
					cargarDesembolso(idSolicitud);
...
function datatablenew(){
    var oTable = $('#tblSolicitud').dataTable({
        "bServerSide": true,
        "sAjaxSource": "/planilla/listar_metas_persona_ajax",
        "bProcessing": true,
        ...
        "fnServerData": function (sSource, aoData, fnCallback, oSettings) {
            ...
            oSettings.jqXHR = $.ajax({
                ...
                "data":{NumeroPagina:iNroPagina,NumeroRegistros:iCantMostrar,
						id_ubicacion:id_ubicacion,id_planilla:id_planilla,id_subplanilla:id_subplanilla,anio:anio,mes:mes,
						_token:_token
                       },
                "success": function (result) {
                    fnCallback(result);
					var id_periodo = $('#id_periodo').val();
					if(id_periodo>0)$('#divConceptoPersona').show();
					else $('#divConceptoPersona').hide();
            ...
            [	
				...
				*/
                ...
                "mRender": function (data, type, row) {
					...
					var html = "";
					if(row.id_persona!= null)id_persona = row.id_persona;
					var html = "<input type='checkbox' class='mov' checked='checked' name='personas[]' value='"+id_persona+"' />";
					...
					if(id_periodo==0)return html;
					else return id_persona;
                ...
                "mRender": function (data, type, row) {
					var regimen = "";
					if(row.regimen!= null)regimen = row.regimen;
					return regimen;
                },
                "bSortable": false,
                "aTargets": [4],
	...
	$('#tblSolicitud tbody').on('click', 'tr', function () {
        ...
        if (anSelected.length != 0) {
			...
			var id_planilla = odtable.row(this).data().id_planilla;
			var id_subplanilla = 3;
			var id_persona = odtable.row(this).data().id_persona;
...
function fn_ListarBusqueda() {
	...
	if(id_subplanilla!=0 && anio!=0 && mes!=0){
		$.ajax({
			url: '/planilla/obtener_periodo/'+id_ubicacion+'/'+id_planilla+'/'+id_subplanilla+'/'+anio+'/'+mes,
			dataType: "json",
			success: function(result){
				//if(result!=null)$('#id_periodo').val(result.id);
				//alert(result.id);				
				$("#tblSolicitud tbody").html("");
				if (result!=null && result.id != "") {
					bootbox.alert("El periodo de la empresa seleccionada ya existe en la planilla, subplanilla, año y mes");
					return false;
				}
				//datatablenew();
				cargarMetasPersona(id_ubicacion,id_planilla,id_subplanilla,anio,mes);
				//$('#example-select-all').prop('checked', true);
			}
		});	
...
function cargarMetasPersona(id_ubicacion,id_planilla,id_subplanilla,anio,id_mes){
	//alert("ok");
	//alert(id_ubicacion+','+id_planilla+','+id_subplanilla+','+anio+','+id_mes);
	
	//$('#tblMetasPersona').dataTable().fnDestroy();
    $("#tblSolicitud tbody").html("");
	$.ajax({
			url: "/planilla/listar_metas_persona/"+id_ubicacion+"/"+id_planilla+"/"+id_subplanilla+"/"+anio+"/"+id_mes,
			type: "GET",
			success: function (result) { 
					
					$('#tblSolicitud').dataTable().fnDestroy(); //la destruimos
					$('#tblSolicitud tbody').html("");
	...
	$.ajax({
		...
		success: function(result){
			...
			$(garantia).each(function (ii, obj) {
				...
				newRow +='<select class="form-control form-control-sm id_tipo" id="id_tipo'+ind+'" ind="'+ind+'" tabindex="'+tabindex+'" name="id_tipo[]">'+item_tipo+'</select></td>';
				newRow +='<td><input value="'+obj.descripcion+'" onKeyPress="return soloNumerosMenosCero(event)" type="text" tabindex="'+(tabindex+2)+'" data-toggle="tooltip" data-placement="top" title="Ingresar la cantidad prescrita y presionar Enter para ingresar la cantidad entregada" name="descripcion[]" required="" id="descripcion'+ind+'" class="limpia_text nro_solicitado descripcion input-sm form-control form-control-sm text-left" style="margin-left:4px;" /></td>';
				newRow +='<td><input value="'+obj.valor_actual+'" onKeyPress="return soloNumerosMenosCero(event)" type="text" tabindex="'+(tabindex+2)+'" data-toggle="tooltip" data-placement="top" title="Ingresar la cantidad prescrita y presionar Enter para ingresar la cantidad entregada" name="valor_actual[]" required="" id="valor_actual'+ind+'" class="limpia_text nro_solicitado valor_actual input-sm form-control form-control-sm text-right" style="margin-left:4px;" /></td>';
				if(valorizador==true){
...
function eliminarPersona_(id,estado){
	//alert(id);
	var act_estado = "";
	//var eliminado = "";
	if(estado==1){
...
function eliminarConceptoPersona(id){
	
    $.ajax({
            url: "/planilla/eliminar_concepto_persona/"+id,
            type: "GET",
            success: function (result) {
...
function modalConceptoPersona(id,id_planilla,id_subplanilla){
	
	$(".modal-dialog").css("width","85%");
	$('#openOverlayOpc .modal-body').css('height', 'auto');

	$.ajax({
			url: "/planilla/modal_concepto_persona/"+id+"/"+id_planilla+"/"+id_subplanilla,
			type: "GET",
			success: function (result) {  
					$("#diveditpregOpc").html(result);
					$('#openOverlayOpc').modal('show');
