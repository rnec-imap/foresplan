...
function cargarTablaUbicacionCliente(id_cliente,tabla,columna){

    $("#tblCronograma tbody").html("");
	$.ajax({
			url: "/maestro/obtener_ubicacion_maestro_cliente/"+id_cliente+"/"+tabla+"/"+columna,
			type: "GET",
			success: function (result) {  
					//$('#id_solicitud').val(idSolicitud);
...
function fn_save_desembolso(id_estado){
    ...
    $.ajax({
            ...
            success: function (result) {
					...
					cargarTablaUbicacionCliente(id_cliente,tabla,columna);
					/*
					$('#tblProductos tbody').html("");
...
function datatablenew(){
    var oTable = $('#tblSolicitud').dataTable({
        "bServerSide": true,
        "sAjaxSource": "/maestro/listar_ubicacion_maestro_ajax",
        "bProcessing": true,
        ...
        "aoColumnDefs":
            [	
                ...
                "mRender": function (data, type, row) {
                	var tabla = "";
					if(row.tabla!= null)tabla = row.tabla;
					return tabla;
                },
                "bSortable": false,
                "aTargets": [0]
                ...
                "mRender": function (data, type, row) {
                	var columna = "";
					if(row.columna!= null)columna = row.columna;
					return columna;
                },
                "bSortable": false,
                "aTargets": [1]
	...
	$('#tblSolicitud tbody').on('click', 'tr', function () {
        ...
        if (anSelected.length != 0) {
			...
			var tabla = odtable.row(this).data().tabla;
			var columna = odtable.row(this).data().columna;
			var id_cliente = odtable.row(this).data().id_cliente;
			//cargarDesembolso(idSolicitud);
			cargarTablaUbicacionCliente(id_cliente,tabla,columna);
        }
    });
