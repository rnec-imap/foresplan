...
function datatablenew() {
    var oTable = $('#tblPlanilla').dataTable({
        "bServerSide": true,
        "sAjaxSource": "/planilla/listar_planilla_resumen_ajax",
        "bProcessing": true,
        ...
        "aoColumnDefs": [{
			...
			/*
            {
                "mRender": function(data, type, row) {
                    ...
                    html += '<a href="/boleta_vista_previa/' + $('#anio').val() + '/' + $('#mes').val() + '/' + row.id + '" class="btn btn-sm btn-success" target="_blank"><i class="fa fa-search"></i></a>';
                    html += '<a href="/boleta_pdf/' + $('#anio').val() + '/' + $('#mes').val() + '/' + row.id + '" class="btn btn-sm btn-success" target="_blank"><i class="fa fa-download"></i></a>';
                    return html;
                },
                "bSortable": false,
                "aTargets": [7],
    ...
    $('#tblPlanilla tbody').on('click', 'tr', function() {
		...
		if (anSelected.length != 0) {
			...
			var id_periodo = odtable.row(this).data().id_periodo;
			var persona = odtable.row(this).data().id;			
			cargarResumenConcepto(id_periodo, persona);
			//cargarIngreso(id_periodo, persona);
			//cargarEgreso(id_periodo, persona);
			//cargarAportaciones(id_periodo, persona);
            //cargarConcepto(id_periodo, persona);
        }
    });
}

function cargarResumenConcepto(id_periodo, persona){
	
    $("#divResumenConcepto").html("");
	$.ajax({
			url: "obtener_concepto_planilla_resumen/"+id_periodo+"/"+persona,
			type: "GET",
			success: function (result) {  
				$("#divResumenConcepto").html(result);
			}
	});
...
function cargarIngreso(id_periodo, persona){
	...
	$.ajax({
			url: "obtener_concepto_planilla_resumen/"+id_periodo+"/"+persona+"/1",
			type: "GET",
			success: function (result) {  
...
function cargarEgreso(id_periodo, persona){
	...
	$.ajax({
			url: "obtener_concepto_planilla_resumen/"+id_periodo+"/"+persona+"/2",
			type: "GET",
			success: function (result) {  
...
function cargarAportaciones(id_periodo, persona){
	...
	$.ajax({
			url: "obtener_concepto_planilla_resumen/"+id_periodo+"/"+persona+"/4",
			type: "GET",
			success: function (result) {  
